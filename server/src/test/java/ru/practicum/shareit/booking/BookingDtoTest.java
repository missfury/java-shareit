...
class BookingDtoTest {
    ...
    void bookingCreationDtoTest() throws IOException {
        ...
        BookingDto bookingCreationDto = BookingDto.builder()
                ...
                .start(now.truncatedTo(ChronoUnit.SECONDS).plusDays(1))
                .end(now.truncatedTo(ChronoUnit.SECONDS).plusDays(2))
                .build();
        ...
        Assertions.assertThat(result).extractingJsonPathStringValue("$.start")
                .isEqualTo((now.truncatedTo(ChronoUnit.SECONDS).plusDays(1)).format(formatter));
        Assertions.assertThat(result).extractingJsonPathStringValue("$.end")
                .isEqualTo((now.truncatedTo(ChronoUnit.SECONDS).plusDays(2)).format(formatter));
    }
